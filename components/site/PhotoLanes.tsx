...
function LaneSet({ shots, inert }: { shots: readonly Shot[]; inert?: boolean }) {
  return (
    <div className="event-lane-set" aria-hidden={inert || undefined}>
      {shots.map((shot) => (
        <figure
          key={`${inert ? "dup" : "src"}-${shot.src}`}
          className="event-lane-card"
        >
          <Image
            src={shot.src}
            alt=""
...
function Lane({ shots, reverse }: { shots: readonly Shot[]; reverse?: boolean }) {
  return (
    <div className={reverse ? "event-lane is-reverse" : "event-lane"}>
...
/** Two marquees running in opposite directions, just below the hero. */
export function PhotoLanes() {
  return (
    <section
