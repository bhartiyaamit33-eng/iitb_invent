/**
 * IIT Bombay Online Pay (OP) — not a direct PayU merchant SDK.
 * OP forwards to PayU / SBI IB / Canara / SBIePay and credits an IITB account.
 */

export type OnlinePayEnv = "test" | "live" | "mock";
...
const LIVE_ACCESS =
  "https://onlinepay.iitb.ac.in/commJsp/v2_accessPoint.jsp";
const TEST_ACK =
...
export function onlinePayConfig(): OnlinePayConfig {
  const raw = (process.env.ONLINEPAY_ENV ?? "test").trim().toLowerCase();
  const mock =
    raw === "mock" && process.env.NODE_ENV !== "production";
  const env: OnlinePayEnv = mock ? "mock" : raw === "live" ? "live" : "test";
  return {
    env,
    appId: (process.env.ONLINEPAY_APP_ID ?? "").trim(),
    accessPoint: env === "live" ? LIVE_ACCESS : TEST_ACCESS,
    ackUrl: env === "live" ? LIVE_ACK : TEST_ACK,
    mock,
  };
}

export function canMockOnlinePay(): boolean {
  return onlinePayConfig().mock;
}

/** OP's sample joins fields with %26 instead of encoding '='. */
...
export function paymentRequestUrl(opts: {
  ...
  const { accessPoint } = onlinePayConfig();
...
export async function acknowledgeOnlinePay(opts: {
  ...
  const { ackUrl, mock } = onlinePayConfig();
  if (mock) return { ok: true };
  if (!opts.transId) return { ok: false, error: "missing transId" };

  const url = `${ackUrl}?transId=${encodeURIComponent(opts.transId)}&requestType=${opts.requestType}`;
  try {
    ...
    if (!res.ok) {
      return { ok: false, error: `ACK HTTP ${res.status}` };
    }
    return { ok: true };
  } catch (err) {
    return {
