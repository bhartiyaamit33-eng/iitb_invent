...
export function colloquiumApplicationCopyEmail(opts: {
  ...
  const html = layout(
    ...
    `<p>Hi ${escapeHtml(opts.name)},</p>
     <p>We received your application for <strong>${escapeHtml(opts.eventName)}</strong>. Organisers will review submissions and write to this email.</p>
...
We received your application for ${opts.eventName}.
