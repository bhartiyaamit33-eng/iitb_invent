...
import { processPayUCallback } from "@/lib/conference-payu";
...
function payRedirect(token: string | null, outcome: string) {
  ...
  if (!token) {
    return NextResponse.redirect(`${origin}/conference?payu=${outcome}`, 303);
  }
  return NextResponse.redirect(
    `${origin}/conference/pay/${encodeURIComponent(token)}?payu=${outcome}`,
    303,
  );
}
...
export async function GET() {
  return NextResponse.redirect(`${siteOrigin()}/conference`, 303);
}
